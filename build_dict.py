...
def iterate_input_sentences(input_file, header):
...
def build_dict(input_file, header=True):
    res = {}
    next_token_id = 0
    for tokens in iterate_input_sentences(input_file, header=header):
        for token in tokens:
            if token not in res:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-header", action="store_true", default=False, help="Input file do not have header")
    ...
    dict_data = build_dict(args.input, header=not args.no_header)
    ...
    if args.train:
        ...
        with open(args.train, "wb+") as fd:
            for sentence in iterate_input_sentences(args.input, header=not args.no_header):
                s_len = len(sentence)
